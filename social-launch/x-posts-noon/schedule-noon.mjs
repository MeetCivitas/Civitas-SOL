...
// ----- load posts -----
const allPosts = JSON.parse(readFileSync(path.join(__dirname, 'posts-noon.json'), 'utf8'));
...
// Reuse the may-2026 login profile so user doesn't log in again.
const userDataDir = path.join(__dirname, '..', 'x-posts-may-2026', 'browser-data');
...
for (const p of posts) {
  ...
  try {
    ...
    // Detect free-tier scheduled-post cap: X shows "The content of your post is invalid"
    const isCap = await page.evaluate(() => /content.*post.*invalid/i.test(document.body.innerText)).catch(() => false);
    const status = isCap ? 'blocked-cap' : 'error';
    console.log(`  ✗ ${p.id} (${status}): ${e.message.split('\n')[0]}`);
    log[p.id] = { status, error: e.message, ts: new Date().toISOString() };
    writeLog();
    if (isCap) {
      console.log('  ▶ X scheduled-post cap reached — stopping. Re-run as earlier posts publish.');
